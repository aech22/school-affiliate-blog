...
- **title に「〜のコツ」「〜の秘訣」「〜に最適な」「幅広い」「様々な」を使わない**（2026-09-07 追加）。
  量産記事のタイトルとして読者に見分けられる。基準は `~/.claude/skills/human-voice/`。
  sourceQuery の語をそのまま含めるルールが優先で、検索語自体にこれらの語が入っている場合は残す。
