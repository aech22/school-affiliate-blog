...
BODY_RULES = """
記事の構成（この順序を守る）:
- **結論を最初に書く。** 導入の最初の2〜3文で、この記事の答えを出す。
  誰にどれを薦めるのか、読者は何をすればいいのかを、読み進めなくても分かる形で先に置く。
  背景説明・前置き・問いかけ（「〜ではないでしょうか」）から始めない。
- 結論のあとに根拠を置く。順序は「比較・並べての検討」→「具体的な場面」→「補足や注意点」。
- **一次体験を装わない。** 「使ってみた」「受講した」「実際に試した」とは書かない。
  書き手は受講も購入もしていないので、体験談を書くと事実と違う主張になる。
  具体を出すときは、想定される読者の状況として書く
  （例:「平日は22時まで働いていて、週末しか机に向かえない人なら」）。

本文の厳守ルール:
- 金額・率（円・万円・%）は、渡された「使ってよい事実」に載っている値だけを書く。
  載っていない金額・率は、概算・言い換え・「約」付きも含めて一切書かない。
- **台帳にある金額は「上限◯円」と、上限であることが分かる形で書く。**
  「◯円もらえる」「◯円が支給される」のように、読者が必ず受け取る額のようには書かない。
  実際に受け取る額は受講する講座と本人の要件で変わるため、上限であることを外さない。
- **台帳の数字から先の計算をしない。** 自己負担額・実質価格・合計・差額・月あたりの額などを
  自分で計算して書かない。金額どうしを足したり引いたり、受講料に給付率を掛けたりしない。
  計算結果は台帳に無い数値なので、書いた時点で記事は破棄される。
  自己負担に触れたいときは「実際の負担額は講座の受講料によって変わる」と書き、数字を出さない。
...
「AIが書いた」と読者に気づかれないためのルール（2026-09-07 追加。基準の正本は
`~/.claude/skills/human-voice/`。既存記事134本の実測で最も多く出た言い回しを禁止する）:
- **記事そのものに言及しない。** intro・body・outro のどこにも
  「この記事では」「本記事では」「本稿では」を書かない。これは実測で最も多く出た痕跡で、
  読者が量産記事だと判断する一番の手がかりになる。
  記事の中身を予告する代わりに、その中身を直接書き始める。
  ✗「この記事では、対象講座の探し方から申請までの流れを説明します。」
  ✓「対象講座は厚生労働省の検索システムで調べる。ここに載っていない講座は給付の対象外になる。」
- 次の言い回しも使わない。
  「本記事では」「この記事では」「〜を解説していきます」「今回は〜を紹介します」
  「〜が重要です」「〜が大切です」「〜が求められます」「〜がポイントです」
  「〜の秘訣」「〜のコツ」「〜に最適です」「ぴったり」「〜を実現します」
  「ニーズに応える」「幅広い」「様々な」「多岐にわたる」「近年」「昨今」
  「いかがでしたか」「ぜひ〜してみてください」「〜と言えるでしょう」「〜ではないでしょうか」
  「個人差があります」「自分に合ったものを選びましょう」
- 「重要です」「大切です」と書きたくなったら、その文自体を消す。
  なぜそうなのかを事実の文で書くか、書けないなら何も書かない。
- 「〜しましょう」「〜してください」と読者に呼びかける文は、記事全体で1つまで。
- H2ごとの長さを揃えない。短い節が1つあってよい。
- 本文（body）の中の箇条書きを、どれも3項目に揃えない（2項目・4項目を混ぜる）。
  ※ items の pros はスキーマどおり3つで固定。この行は本文の箇条書きだけの話。
- 太字は本文1000字あたり3個まで。
- ⚠️ 具体を出せないときに、数値・体験・事例を創作して埋めてはいけない。
  上の「使ってよい事実」に無いことは書かず、その箇所は短いまま終える。

検索から来る読者に届かせるルール（2026-09-06 追加）:
- テーマの中心になる言葉は、導入と最初の H2 の中で言い換えずにそのまま使う。
  同じ意味の別表現に置き換えると、その言葉で探している読者に届かない。
- H2 は、読者が検索するときの言い回しに寄せる（「〜とは」「〜の条件」「〜の選び方」
  「〜の始め方」「〜はいくらかかるか」など）。ただし煽り文句や体言止めの見出しにはしない。
- H2 の内容は見出しが約束したことに答える。見出しと中身をずらさない。
"""

SCHEMA_LINE = """出力は JSON のみ。コードフェンス(```)で囲まない。前置き・後書きを付けない。
スキーマ（items の要素数と順序は入力のサービスと必ず一致させる）:
{"description":"この記事だけの要約。全角90〜120字。この記事を探している人が使う言葉を含める（サイト全体の説明にしない）","intro":"導入3〜4文。1文目は読者が知りたい答えそのものから始める。記事や書き手に言及する語（「この記事では」等）で始めない","body":"Markdown本文。H2(##)を3〜5個。各H2の下は2〜4段落","outro":"まとめ3〜4文","items":[{"pros":["...","...","..."],"cons":"...","target":"..."}]}"""

SYSTEM_COMPARE = """あなたは日本語のサービス比較記事のライターです。
...
def _llm_prose(services: list, theme: str, facts: list, topic_type: str, products: list | None = None) -> dict:
    ...
    user = (f"テーマ: {theme}\n"
            ...
            f"**この一覧の値をそのまま書くだけにしてください。**金額は「上限◯円」と上限であることが"
            f"分かる形で書き、そこから先の計算（自己負担額・実質価格・合計・差額）はしないでください。"
            f"一覧に無い数値は、計算結果であっても書いた時点で記事は破棄され、公開されません。\n\n"
