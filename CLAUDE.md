...
⚠️ **金額（円・万円）は台帳にあっても本文に書かせない**（2026-09-05 方針変更）。給付の上限額は制度で決まっていても、実際に受け取る額は受講する講座と本人の要件で変わる。本文に具体的な金額を書くと、読者が自分に当てはまる額として読んでしまう。金額は各自が公式サイトや窓口で確認するものとして扱い、本文は「支給額は公式の案内で確認」と促す。

**率（%）は従来どおり台帳照合**（`numbers[]` に無ければ破棄）。給付率は制度で決まっていて個人差がないため。
