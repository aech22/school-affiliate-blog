...
### カテゴリ（5種・gender軸なし・既存slugは変えない）

`programming`（プログラミング・IT）/ `career`（転職・働き方）/ `qualification`（学び・スキル講座）/ `language`（語学・留学）/ **`workspace`（働く環境・2026-09-06 追加）**

**`workspace` は「仕事のとなりにある生活」の枠。** 在宅の机まわり、オンライン面接の映り方、学習を続けるための道具。2026-09-05 の「総合化しない」決定を 2026-09-06 にユーザーが覆し、日常系も扱うことにしたのを受けて新設した。ただし**仕事:日常 = 7:3** で、日常が多いブログにはしない（`replenish.py` の `LIFESTYLE_SLUGS` と `LIFESTYLE_MAX_RATIO=0.3` が補充時に機械的に効かせる。上限は切り捨てなので8件のキューなら日常は2件まで）。

⚠️ **記事0件のカテゴリはページを生成しない。** `taxonomy.ts` の `activeCategories()` が「記事があるカテゴリ」だけを返し、カテゴリページ・ヘッダー・フッター・サイドバー・トップ・404 の全てがこれを使う。カテゴリ新設が 2026-09-05 に却下された理由（空ページを公開して後で畳むと404になる）を、手でフラグを切り替えずに解いたもの。**`taxonomy.ts` に定義を置いても、記事が入るまで URL は生まれない。** 新しくカテゴリ導線を足す場所を作るときは、`CATEGORIES` ではなく `activeCategories()` を使うこと（`CATEGORIES` を直接 map するとリンク切れになる。実際 `BaseLayout.astro` のヘッダーとフッターで踏んだ）。
...
### AI要約リスクのゲート（2026-09-07 追加）

`scripts/aio_risk.py` が、補充候補の `sourceQuery` を「AI要約で答えが完結する検索語」かどうかで見る。テストは `python3 scripts/test_aio_risk.py`（10件）。

**なぜ要るか**: Pew Research Center の実測（900人・68,879検索・2025年3月）で、AI要約が出た検索の従来結果クリック率は**8%**、出ない検索は**15%**だった。要約内リンクのクリックは1%。順位を取っても流入が約半分になる。

⚠️ **軸は記事の型ではなく検索語に置く。** Seer Interactive の49,353クエリ分類ではAI要約の出現率は情報意図36%・商業意図8%・取引意図5%だが、**形式別では比較型（AとBどっち・vs）が95.4%、質問形85.9%、レビュー形86.3%**。つまり「compare にすれば避けられる」は成立しない。判定するのは `cand["type"]` ではなく `cand["sourceQuery"]` である。

**落とし方は弱くしてある。** ハード却下は `validate()` 内の定義型（「〜とは」「意味」「読み方」）だけ。それ以外の情報意図は `enforce_intent_ratio()` がキューの3割までに抑える（`enforce_lifestyle_ratio` と同じ仕組み・同じ理由で比率は補充後のキュー全体で測る）。一律に弾くと compare 型そのものが成立せずキューが枯れるため。AFFILIATE.md ハマりどころ22（薬機法ゲートが公開済み記事3本を誤検出で落とした）と同型の失敗を避けている。

⚠️ **取引マーカーに制度名の一部（給付・支給・還付）を入れてはいけない。** 「教育訓練給付金とは」が部分一致だけで取引意図に化け、qualification カテゴリがまるごと素通りする。初回実装で実際に踏み、`test_subject_noun_does_not_leak_into_transactional` が回帰テストとして固めてある。**軸は主題が何かではなく、読者が何をしようとしているか。**

補充の出力には分類が出る（`← 「教育訓練給付金 申請」（transactional）`）。観測できないゲートは維持されないため。
